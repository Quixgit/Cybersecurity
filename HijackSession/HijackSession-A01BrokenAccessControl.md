# Звіт з практичної роботи: A01 Broken Access Control — Hijack a session
...
1. **Методологія атаки застосована коректно**: аналіз формату кукі (номер сесії + timestamp), виявлення закономірності зростання, спроби передбачення пропущених значень через Burp Repeater та Intruder, спроба залучення другої реальної паралельної сесії — усе це стандартний і правильний підхід до session-based brute force атаки, описаний і в референсному матеріалі викладача.
2. **Ключова відмінність від референсного прикладу**: у прикладі викладача сервер, ймовірно, обслуговував **багато реальних одночасних користувачів** (груповий навчальний сервер), тому майже будь-який номер сесії в діапазоні відповідав чиїйсь реальній активній сесії — перебір швидко знаходив збіг. У нашому локальному однокористувацькому Docker-контейнері реальних паралельних сесій критично мало, тому ймовірність влучання в реальний, ще дійсний токен значно нижча.
3. Навіть підстановка **точного, щойно виданого** значення від другого реального користувача не спрацювала — це свідчить або про дуже короткий час життя токена (він міг «протухнути» ще до відправки через затримки ручного процесу), або про додаткову логіку перевірки на сервері (наприклад, заборону використання власної щойно виданої сесії чи додаткову прив'язку до конкретного HTTP-з'єднання), яку не вдалося встановити без доступу до вихідного коду цього білда WebGoat.
4. Для практичного успішного проходження цього уроку в реальних умовах потрібне середовище з кількома **справді одночасно активними** користувачами (наприклад, спільний навчальний сервер або кілька синхронізованих скриптованих клієнтів, що діють без затримок), а не покроковий ручний процес.

Незважаючи на відсутність фінального «Congratulations», робота демонструє повне розуміння вразливості Broken Access Control через передбачувані ідентифікатори сесій та практичне володіння інструментарієм Burp Suite (Proxy, Repeater, Intruder) для тестування подібних сценаріїв.
